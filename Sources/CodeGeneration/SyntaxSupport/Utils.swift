...
public func lowercaseFirstWord(name: String) -> String {
  ...
  return name.prefix(wordIndex).lowercased()
    + name[name.index(name.startIndex, offsetBy: wordIndex)..<name.endIndex]
}
...
extension SwiftSyntax.Trivia {
  ...
  public init(
    ...
    separator: SwiftSyntax.Trivia = SwiftSyntax.Trivia(pieces: [
      TriviaPiece.newlines(1), TriviaPiece.docLineComment("///"), TriviaPiece.newlines(1),
    ])
  ) {
